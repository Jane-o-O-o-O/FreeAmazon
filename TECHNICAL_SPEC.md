...
- 支持 Amazon 主图反向图片搜索、1688 关键词搜索、候选商品字段标准化
- 支持 SiliconFlow 将 Amazon 英文标题和类目生成 1688 中文关键词
- 默认仍启用 mock 模式，配置 `APIFY_API_TOKEN` 且设置 `APIFY_USE_MOCK=false` 后走真实 Apify Actor
- Apify 默认 Actor：
  - 反向图片搜索：`dev00/alibaba-1688-aliexpress-reverse-image-search-api`
  - 关键词搜索：`ecomscrape/1688-product-search-scraper`
- 反向图片搜索 Actor 可能返回 1688、Alibaba、AliExpress 混合候选，后端会按商品 URL 识别平台并保留原始字段
...
5. 调用 Apify 反向图片搜索 Actor，以 Amazon 主图寻找 1688 相似货源
6. 使用 Amazon 标题生成中文关键词，并调用 Apify 1688 关键词搜索 Actor
7. 合并图片搜索和关键词搜索结果
...
用户输入 Amazon URL
  ...
  -> 下载或缓存 Amazon 主图/副图
  -> 调用 Apify 1688 图片搜索 Actor
  -> 将 Amazon 标题翻译/改写为中文关键词
  -> 调用 Apify 1688 关键词搜索 Actor
  -> 合并候选 1688 商品
  -> 标准化候选详情和供应商信息
  -> CLIP 计算图片相似度
  -> 计算综合评分
