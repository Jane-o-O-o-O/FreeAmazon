...
- 后端已实现 Apify REST 客户端：`apps/api/app/services/apify_client.py`
- 1688 搜源链路已接入 `Source1688Service`
- 当前主链路使用 1688 关键词搜索，候选商品字段会在后端标准化
- 旧的 Amazon 主图反向图片搜索 Actor 已保留为可选配置，但默认关闭
- 支持 SiliconFlow 将 Amazon 英文标题和类目生成 1688 中文关键词
- 默认仍启用 mock 模式，配置 `APIFY_API_TOKEN` 且设置 `APIFY_USE_MOCK=false` 后走真实 Apify Actor
- Apify 默认 Actor：
  - 主搜源：`ghXSMZcW3GxsCrkiR`（Actor：`zen-studio/1688-wholesale-scraper`）
  - 可选反向图片搜索：默认空；需要时可配置 `APIFY_REVERSE_IMAGE_ACTOR`
- `ghXSMZcW3GxsCrkiR` 支持 `keywords`、`offerIds`、`maxResults`、`sortBy`、`priceMin`、`priceMax`、`minOrderQuantity`、`merchantType`、`province`、`city`
- 该 Actor 采用 Apify 按事件计费，核心成本与抓到的商品数相关，开发验证时应使用较小的 `APIFY_SEARCH_LIMIT`
...
- 能覆盖本项目 MVP 的主要数据需求
- 与 Canopy API 返回的 Amazon 标题、类目、图片可以形成完整链路
- 有免费计划/试用入口，适合先跑 MVP
- Actor 可替换，后续可按稳定性和成本切换供应商
...
5. 使用 Amazon 标题和类目生成中文关键词
6. 调用 Apify `ghXSMZcW3GxsCrkiR` 搜索 1688 候选货源
7. 合并多关键词搜索结果
8. 标准化候选 1688 商品、价格、起订量和供应商字段
...
用户输入 Amazon URL
  ...
  -> 提取 Amazon 主图/副图用于展示和后续相似度
  -> 将 Amazon 标题翻译/改写为中文关键词
  -> 调用 Apify 1688 Wholesale Scraper
  -> 合并候选 1688 商品
  -> 标准化候选详情和供应商信息
  -> CLIP 计算图片相似度
  -> 计算综合评分
...
Amazon URL
  ...
  -> Canopy API
  -> AmazonProduct
  -> SiliconFlow keywords
  -> Apify 1688 wholesale scraper
  -> CandidateSourceItem
  -> CLIP rerank
  -> RankedSourceItem
```
...
- 调用 Apify `ghXSMZcW3GxsCrkiR`
- 将 SiliconFlow 生成的多个中文关键词批量传给 Apify
- 合并多关键词搜索结果
- 标准化 1688 商品、价格、起订量、销量和供应商字段

关键函数：

```python
search_candidates(amazon_product: AmazonProduct, filters: dict) -> list[SourceItem]
normalize_apify_item(raw: dict) -> SourceItem
```
...
当前选择 Apify 作为 1688 主数据源。

注意：

- 1688 数据接口可能存在字段变化、价格变化、登录风控
- 正式商用前需要评估 Apify Actor 的稳定性、计费模型和服务条款
...
- 接入 Apify `ghXSMZcW3GxsCrkiR`
- 使用 Amazon 标题、类目和 SiliconFlow 生成的中文关键词搜索 1688 商品
- 保存候选商品基础数据
- 前端展示候选列表
...
3. Canopy API 和 Apify 必须封装在独立 service 中，方便替换供应商
